...
async function testParallelProgress() {
  ...
  for (let i = 0; i < threadCount; i++) {
    ...
    if (progress) {
      ...
      console.log(
        `Thread ${i}: ${progress.processed} / ${progress.total} ${isCorrect ? "✓" : "✗"}`,
      );
      if (!isCorrect) {
        allThreadsCorrect = false;
        console.log(
          `  Expected: ~${expectedPerThread}, Got: ${progress.total}`,
        );
      }
    } else {
  ...
  console.log(
    `Total global progress messages: ${globalProgressMessages.length}`,
  );
