...
export function savePerformanceConfig(config: Partial<PerformanceConfig>): void {
  if (typeof localStorage !== 'undefined') {
    ...
    localStorage.setItem('csv-diff-performance-config', JSON.stringify(fullConfig));
  }
}
 ...
 */
export function applyPerformanceProfile(profile: keyof typeof PERFORMANCE_PROFILES): void {
